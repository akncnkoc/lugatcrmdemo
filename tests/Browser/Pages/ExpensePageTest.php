...
class ExpensePageTest extends DuskTestCase
{
  ...
  public function test_user_can_show_expenses(): void
  {
    $this->browse(function (Browser $browser) {
      $browser
        ->visitRoute('expense.index')
        ->assertRouteIs('expense.index')
  ...
  public function test_user_can_add_expense(): void
  {
    $this->browse(function (Browser $browser) {
      $browser->refresh()
        ...
        ->pause(2000)
        ->clickAtXPath('//*[@id="kt_toolbar_container"]/div[2]/button')
        ...
        ->pause(2000)
        ->keys('#create_form textarea[name="comment"]', [WebDriverKeys::LEFT_CONTROL . "a" . WebDriverKeys::DELETE],
          "Yorum")
        ->pause(2000)
  ...
  public function test_user_can_update_expense(): void
  {
    $this->browse(function (Browser $browser) {
      $browser->refresh()
        ...
        ->pause(2000)
        ->keys('#edit_form input[name="date"]', [WebDriverKeys::LEFT_CONTROL . "a" . WebDriverKeys::DELETE],
          date('d-m-Y'))
        ->pause(2000)
        ->keys('#edit_form textarea[name="comment"]', [WebDriverKeys::LEFT_CONTROL . "a" . WebDriverKeys::DELETE],
          "Yeni Yorum Geldi")
        ->pause(2000)
