<x-modal.modal id="edit_modal" size="modal-fullscreen">
  <x-slot name="title">Giden İrsaliye Düzenle</x-slot>
  <x-slot name="body">
    ...
    <x-form.form id="edit_form">
      <div class="row row-cols-2">
        ...
        <x-form.select name="customer_id" label="Müşteri" placeholder="Müşteri"
                       :asyncload="route('customer.select')"
...
@push('customscripts')
  <script>
    ...
    var {
      ...
      $.ajax({
        url: "{{ route('outgoing-waybill.update') }}",
        type: "POST",
        data: data,
      ...
      $(form).on('change', '.customer_id_edit_select', function (e) {
        let name = $(e.target).attr('name');
        validator.revalidateField(name);
      });
    ...
    $("#edit_modal").on('shown.bs.modal', function (e) {
      ...
      $.ajax({
        url: "{{ route('outgoing-waybill.get') }}",
        data: {
          id
        },
        type: "POST",
        ...
        success: function (data) {
          ...
          let customer_edit_option = new Option(data.customer.name + " " + data.customer.surname, data.customer.id, false, true);
          $(editForm).find('select[name="customer_id"]').html(customer_edit_option);
          if (data.products && data.products.length > 0) {
            ...
            data.products.map((item, index) => {
              ...
              $(clone).find('input[name="id"]').attr('name', 'waybill_product_edit[' + (index + 1) + '][id]').val(item.id);
