  ...
  public function created(Cargo $cargo): void
  {
    if ($cargo->paided) {
      $safe_log = $cargo->safe_log()->create([
        'price'            => $cargo->price,
        'content'          => sprintf("%s kargo firmasına %s takip kodlu kargo için %s %s ödendi",
          $cargo->cargo_company->name, $cargo->tracking_number, $cargo->price, $cargo->safe->currency->code),
        'cash_register_id' => 1,
        'commission'       => 0,
        'safe_id'          => $cargo->safe_id,
        'process_type'     => AppHelper::PRODUCT_CARGO_OUTPUT,
