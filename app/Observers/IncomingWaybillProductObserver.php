...
class IncomingWaybillProductObserver
{
  ...
  {
    $productLast = IncomingWaybillProduct::orderBy('id', 'desc')
      ->first();
    if ($productLast != null) {
  ...
  public function created(IncomingWaybillProduct $waybillProduct)
  {
    $waybillProduct->product->productLog()->create([
      'content'      => sprintf("%s adındaki üründen %s tarihinde %s ürün kodlu ürün giriş yaptı.", $waybillProduct->product->name, AppHelper::convertDate($waybillProduct->waybill->waybill_date), $waybillProduct->product_code),
